...
import { FTContractMetadata } from "./ft-metadata";
@NearBindgen({ requireInit: true })
export class FungibleToken {
	accounts: LookupMap = new LookupMap("accounts");
	totalSupply = "0";
	metadata: FTContractMetadata;

	@initialize({})
	init({ owner_id, total_supply, metadata }) {
	...
	internalGetBalance(accountId): number {
		near.log(`Getting balance for ${accountId}`)
		assert(this.accounts.containsKey(accountId), `Account ${accountId} is not registered`);
		return this.accounts.get(accountId) as number;
	}

	internalDeposit(accountId, amount) {
		near.log(`Depositing Tokens for ${accountId}`)
	...
	internalWithdraw(accountId, amount) {
		near.log(`Withdrawing Tokens for ${accountId}`)
		let balance = this.internalGetBalance(accountId);
		let newBalance = BigInt(balance) - BigInt(amount);
	...
	ft_balance_of({ account_id }) {
		near.log(`Getting balance for ${account_id}`)
