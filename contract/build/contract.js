...
function internalRemoveTokenFromOwner(contract, accountId, tokenId) {
  ...
  assert(tokenSet !== null, "Token should be owned by the sender"); //we remove the the token_id from the set of tokens
...
function internalTransfer(contract, senderId, receiverId, tokenId, approvalId, memo) {
  ...
  assert(token !== null, "no token found"); //if the sender doesn't equal the owner, we check if the sender is in the approval list

  if (senderId != token.owner_id) {
    //if the token's approved account IDs doesn't contain the sender, we panic
    assert(token.approved_account_ids.hasOwnProperty(senderId), "Unauthorized"); // If they included an approval_id, check if the sender's actual approval_id is the same as the one included

    if (approvalId != null) {
      //get the actual approval ID
      let actualApprovalId = token.approved_account_ids[senderId]; //if the sender isn't in the map, we panic

      assert(actualApprovalId !== null, "Sender is not approved account"); //make sure that the actual approval ID is the same as the one provided
...
function internalNftApprove({
  ...
  */
  ...
  let token = contract.tokensById.get(tokenId);
  assert(token !== null, "no token"); //make sure that the person calling the function is the owner of the token
...
function internalNftIsApproved({
  ...
  let token = contract.tokensById.get(tokenId);
  assert(token !== null, "no token"); //get the approval number for the passed in account ID
...
function internalNftRevoke({
  ...
  let token = contract.tokensById.get(tokenId);
  assert(token !== null, "no token"); //get the caller of the function and assert that they are the owner of the token
...
function internalNftRevokeAll({
  ...
  let token = contract.tokensById.get(tokenId);
  assert(token !== null, "no token"); //get the caller and make sure they are the owner of the tokens
...
function internalNftPayout({
  ...
  let token = contract.tokensById.get(tokenId);
  assert(token !== null, "no token"); //get the owner of the token
...
let Contract = (_dec = NearBindgen({
  requireInit: true
}), _dec2 = call({}), _dec3 = view({}), _dec4 = call({}), _dec5 = call({}), _dec6 = call({}), _dec7 = view({}), _dec8 = call({}), _dec9 = view({}), _dec10 = call({}), _dec11 = call({}), _dec12 = call({}), _dec13 = view({}), _dec14 = view({}), _dec15 = view({}), _dec16 = view({}), _dec17 = view({}), _dec(_class = (_class2 = class Contract {
  /*
